...
class CompraController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($detallesRequest as $detalle) {
                ...
                // Registrar movimiento en el inventario (nueva funcionalidad)
                Inventario::create([
    ...
    public function update(Request $request, Compra $compra)
    {
        try {
            ...
            DB::transaction(function () use ($request, $compra) {
                ...
                // Primero revertimos los movimientos de inventario anteriores (nueva funcionalidad)
                ...
                foreach ($request->detalles as $detalleData) {
                    ...
                    // Registrar nuevo movimiento en el inventario (nueva funcionalidad)
                    Inventario::create([
                        'producto_id' => $detalleData['producto_id'],
                        'compra_id' => $compra->id,
    ...
    public function destroy(Compra $compra)
    {
        DB::transaction(function () use ($compra) {
            // Revertir movimientos de inventario (nueva funcionalidad)
            foreach ($compra->detalleCompras as $detalle) {
                $producto = Producto::find($detalle->producto_id);
                if ($producto) {
                    $producto->decrement('stock', $detalle->cantidad);
            ...
            // Eliminar la compra y sus detalles
            $compra->detalleCompras()->delete();
            $compra->delete();
        });
