...
interface Producto {
    id: number;
    nombre: string;
    precio_compra: number; // Campo para precargar el precio unitario
}
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Crear Detalle de Compras', href: '#' },
];

// Inicialización del formulario con valores por defecto
const form = useForm({
    compra_id: '',
    producto_id: '', // Este será un string debido al v-model en <select>
    cantidad: 1,
    precio_unitario: 0,
    subtotal: 0,
    impuesto_iva: 0,
...
const displayTotalPagar = ref('0'); // Se mantiene para la visualización
...
const formatNumberInput = (field: 'precio_unitario') => {
    ...
    if (field === 'precio_unitario') {
        ...
        return; // Solo aplica para precio_unitario por ahora
    }
...
<template>
    <Head title="Crear Detalle de Compra" />
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block font-medium text-gray-700 dark:text-gray-300 mb-2" for="venta_id">ID de Compra</label>
                        ...
                        <div class="flex flex-col md:flex-row md:space-x-4 space-y-4 md:space-y-0 pt-6">
                            <button type="submit" class="flex-1 md:flex-none px-6 py-3 rounded-lg bg-cyan-700 hover:bg-cyan-400 text-white font-medium shadow-lg shadow-cyan-500/20 transition-all" :disabled="form.processing">
                                ...
                                <span v-else>Guardar Detalle de compra</span>
                            </button>
