...
// Roles actuales
const rols = computed(() => props.rols.data);

// Búsqueda local
const rolsFiltrados = computed(() =>
  rols.value.filter((rol) =>
...
// Breadcrumbs
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Roles', href: '/rols' }];

// Eliminar rol
const deleteRol = async (id: number) => {
  if (!window.confirm('¿Seguro que quieres eliminar este Rol?')) return;
  router.delete(`/rols/${id}`, {
    preserveScroll: true,
    onSuccess: () => {
      router.visit('/rols', { replace: true });
    },
    onError: (errors) => {
      console.error('Error deleting rol:', errors);
    }
  });
};
</script>
    ...
    <div class="flex flex-col gap-4 p-4 bg-gray-100 dark:bg-gray-900">
      ...
      <div class="flex justify-between items-center flex-wrap gap-2">
        <Button as-child size="sm" class="flex-1 md:flex-none px-6 py-3 rounded-lg bg-cyan-700 hover:bg-cyan-400 text-white font-medium shadow-lg shadow-cyan-500/20 transition-all">
          <Link href="/rols/create"><CirclePlus class="mr-1" /> Crear</Link>
        </Button>
        <input v-model="search" type="text" placeholder="Buscar Roles..." class="px-4 py-2 rounded border border-zinc-300 w-full sm:w-64 dark:bg-zinc-800 dark:text-white"/>
      </div>
      ...
      <div class="overflow-x-auto rounded-xl border border-sidebar-border/70 dark:border-sidebar-border">
        <Table class="min-w-[900px] w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
          ...
          <TableBody>
            <TableRow v-for="rol in rolsFiltrados" :key="rol.id">
              <TableCell>{{ rol.nombre }}</TableCell>
              <TableCell class="text-center space-x-2">
                <Button as-child size="sm" class="bg-blue-500 hover:bg-blue-700 text-white">
                  <Link :href="`/rols/${rol.id}/edit`"><Pencil /></Link>
                </Button>
                <Button size="sm" class="bg-rose-500 hover:bg-rose-700 text-white" @click="deleteRol(rol.id)">
                  <Trash2 />
                </Button>
              </TableCell>
            </TableRow>
      ...
      <div class="flex justify-center mt-4 gap-2 flex-wrap">
        <template v-for="(link, i) in props.rols.links" :key="i">
          <button
            v-if="link.url"
            :class="[
