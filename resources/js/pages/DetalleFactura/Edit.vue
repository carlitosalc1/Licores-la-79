...
const props = defineProps<{
    facturas: Factura[];
    productos: Producto[];
    detalleFactura: DetalleFactura; // Se añade la prop para el detalle de factura existente
}>();

// Breadcrumbs para la navegación
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Detalles de factura', href: '/detalle_facturas' },
    { title: `Editar Detalle: ${props.detalleFactura.id}`, href: '#' },
];

// Inicialización del formulario con los valores del detalleFactura existente
const form = useForm({
    factura_id: props.detalleFactura.factura_id,
    producto_id: String(props.detalleFactura.producto_id),
    cantidad: props.detalleFactura.cantidad,
    precio_unitario: props.detalleFactura.precio_unitario,
    subtotal: props.detalleFactura.subtotal,
    impuesto_iva: props.detalleFactura.impuesto_iva,
    total_pagar: props.detalleFactura.total_pagar,
});
...
const submit = () => {
    ...
    form.put(route('detalle_facturas.update', props.detalleFactura.id), {
        onSuccess: () => {
            router.visit(route('detalle_facturas.index'));
        },
        onError: (errors) => {
            console.error("Error al actualizar el detalle de factura:", errors);
        },
    });
};
</script>
...
<template>
    <Head :title="`Editar Detalle de Factura: ${detalleFactura.id}`" />
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="min-h-screen flex items-center justify-center p-4 bg-gray-100 dark:bg-gray-900">
            <div class="w-full max-w-3xl">
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                    <h1 class="text-2xl font-bold mb-6 text-gray-800 dark:text-white">Editar Detalle de Factura: {{ detalleFactura.id }}</h1>
                    <form @submit.prevent="submit" class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block font-medium text-gray-700 dark:text-gray-300 mb-2" for="factura_id">ID de Factura</label>
                                <select id="factura_id" v-model="form.factura_id"
                                    ...
                                    <option disabled value="">Seleccione una Factura</option>
                                    <option v-for="factura in facturas" :key="factura.id" :value="factura.id">
                                        {{ factura.id }}
                                    </option>
                                </select>
                                <InputError :message="form.errors.factura_id" class="mt-2" />
                            </div>
                        ...
                        <div class="flex flex-col md:flex-row md:space-x-4 space-y-4 md:space-y-0 pt-6">
                            <button type="submit" class="flex-1 md:flex-none px-6 py-3 rounded-lg bg-cyan-700 hover:bg-cyan-400 text-white font-medium shadow-lg shadow-cyan-500/20 transition-all" :disabled="form.processing">
                                ...
                                <span v-else>Actualizar Detalle de Factura</span>
                            </button>
                            <button type="button" @click="router.get(route('detalle_facturas.index'))"
